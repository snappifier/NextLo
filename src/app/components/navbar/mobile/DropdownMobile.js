	...
	return (
		<>
			...
			>
				...
				<button
					className={`absolute top-20 left-4 z-10 p-2 rounded-full bg-slate-100 text-slate-600 hover:bg-slate-200 active:scale-95 transition-all duration-200 ${showMain ? "opacity-0 pointer-events-none" : "opacity-100"}`}
				...
				>
					<svg xmlns="http://www.w3.org/2000/svg" width={20} height={20} viewBox="0 0 24 24">
						<path fill="currentColor"
						      d="m10.8 12l3.9 3.9q.275.275.275.7t-.275.7t-.7.275t-.7-.275l-4.6-4.6q-.15-.15-.212-.325T8.425 12t.063-.375t.212-.325l4.6-4.6q.275-.275.7-.275t.7.275t.275.7t-.275.7z"></path>
					</svg>
				</button>
							...
							>
								{categories.length ? (
									categories.map((cat, i) => (
										<motion.button key={cat?.id ?? i}
										               className="w-full flex items-center justify-between px-4 py-4 rounded-xl bg-slate-50 hover:bg-slate-100 active:bg-slate-200 transition-colors"
										...
										<span className="text-slate-800 text-base font-medium">
											...
											<svg xmlns="http://www.w3.org/2000/svg" width={20} height={20} viewBox="0 0 24 24">
												<path fill="currentColor"
												      d="M10.061 19.061L17.121 12l-7.06-7.061l-2.122 2.122L12.879 12l-4.94 4.939z"></path>
											</svg>
										</motion.button>
									))
								) : (
				...
				<div className="w-full px-5 py-4 bg-linear-to-t from-slate-50 to-white border-t border-slate-100">
					<div className="flex items-center justify-center gap-3">
						...
						<ContactIcon onClick={() => handleCopy(PHONE_NUMBER, "Numer skopiowany!")} label={`Zadzwoń: ${PHONE_NUMBER}`} tooltipText={PHONE_NUMBER}><svg xmlns="http://www.w3.org/2000/svg" width={22} height={22} viewBox="0 0 56 56"><path fill="currentColor" d="M18.156 37.762c6.774 6.773 15.024 12 21.75 12c3.024 0 5.672-1.055 7.805-3.399c1.242-1.383 2.016-3 2.016-4.593c0-1.172-.446-2.297-1.57-3.094l-7.173-5.11c-1.101-.75-2.015-1.125-2.859-1.125c-1.078 0-2.016.61-3.094 1.664l-1.664 1.641a1.26 1.26 0 0 1-.89.375c-.375 0-.704-.14-.961-.258c-1.43-.773-3.914-2.906-6.235-5.203c-2.297-2.297-4.43-4.781-5.18-6.234a2 2 0 0 1-.257-.938c0-.304.093-.61.351-.867l1.64-1.71c1.056-1.079 1.665-2.017 1.665-3.095c0-.843-.375-1.757-1.148-2.859l-5.04-7.102c-.82-1.125-1.968-1.617-3.234-1.617c-1.547 0-3.164.703-4.523 2.04c-2.274 2.18-3.282 4.874-3.282 7.85c0 6.727 5.133 14.884 11.883 21.634"/></svg></ContactIcon>
						<ContactIcon onClick={() => handleCopy(EMAIL, "Email skopiowany!")} label={`Skopiuj email: ${EMAIL}`} tooltipText={EMAIL}><svg xmlns="http://www.w3.org/2000/svg" width={22} height={22} viewBox="0 0 24 24"><path fill="currentColor" d="M22 6c0-1.1-.9-2-2-2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2zm-2 0l-8 5l-8-5zm0 12H4V8l8 5l8-5z"/></svg></ContactIcon>
