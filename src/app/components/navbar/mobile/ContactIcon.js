...
export default function ContactIcon({children, href, label, onClick, tooltipText}) {
	...
	if (onClick) {
		return (
			<button className="outline-none focus-visible:ring-2 focus-visible:ring-slate/30 rounded-xl" type="button" onClick={onClick} aria-label={label}>
				{content}
			</button>
		)
	}
	...
	return (
		<a href={href} className="outline-none focus-visible:ring-2 focus-visible:ring-slate-300 rounded-xl" target="_blank" rel="noopener noreferrer" aria-label={label}>
			{content}
		</a>
	)
