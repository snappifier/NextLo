...
export default function Profile({item}) {
	...
	const handlePointerEnter = () => {
		setArrow(true);
		if (isLg) setHovered(true);
	};

	const handlePointerLeave = () => {
		setArrow(false);
		if (isLg) setHovered(false);
	};

	const handleFocus = () => isLg && setHovered(true);
	const handleBlur = () => isLg && setHovered(false);
	...
	return (
		<motion.div
			className="relative w-full h-70 max-w-xl min-w-85 bg-zinc-100 rounded-xl overflow-hidden select-none flex flex-col justify-between items-center shadow-md"
